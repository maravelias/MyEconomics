...
public class ViewTraderBalances {
    @JmixGeneratedValue @Column(name = "TRADER_ID", nullable = false) @Id private UUID id;

    @InstanceName @Column(name = "TITLE") private String title;

    @Column(name = "BALANCE") private BigDecimal balance;

    @Temporal(TemporalType.DATE) @Column(name = "LATEST_TRANSACTION_DATE") private Date latestTransactionDate;

    @Column(name = "DAYS_SINCE_LAST_TRANSACTION") private Integer daysSinceLastTransaction;
    ...
    public void setLatestTransactionDate(Date latestTransactionDate) {this.latestTransactionDate = latestTransactionDate;}

    public Date getLatestTransactionDate() {return latestTransactionDate;}
