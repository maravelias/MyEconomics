...
@JmixEntity
@Table(name = "MYEC_BANK_TRANSACTION", indexes = {
...
public class BankTransaction extends CommonEntity {
    @OnDeleteInverse(DeletePolicy.DENY) @JoinColumn(name = "BANK_ID", nullable = false) @NotNull @ManyToOne(fetch = FetchType.LAZY, optional = false) private BankAccount bank;
    ...
    public TransactionType getTransactionType() {return transactionType == null ? null : TransactionType.fromId(transactionType);}

    public void setTransactionType(TransactionType transactionType) {this.transactionType = transactionType == null ? null : transactionType.getId();}
    ...
    public void setBank(BankAccount bank) {this.bank = bank;}
}